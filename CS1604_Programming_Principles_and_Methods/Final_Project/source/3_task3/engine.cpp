...
bool performShoot(ostream& os, Field& field, Unit* u, int& choice)
{
    int trow, tcol;
    switch(choice){
    case 0:
...
bool performActions(ostream& os, istream& is, Field& field, Unit* u)
{
    ...
    if (!queryAction(os, is, acts, act)) {
        ...
        if (!is) exit(0);
        if (u -> getMoval()) return performActions(os, is, field, u);
        else return false;
    }
    ...
    case ATTACK:
        if (u -> getMoval()) {
            if (!queryAttack(os, is, choice) || !performAttack(os, field, u, choice)) 
                return performActions(os, is, field, u);
            else return true;
        }
        else {
            if (!queryAttack(os, is, choice)) return false;
            else return performAttack(os, field, u, choice);
        }

    case SHOOT:
        if (u -> getMoval()) {
            if (!queryAttack(os, is, choice) || !performShoot(os, field, u, choice)) 
                return performActions(os, is, field, u);
            else return true;
        }
        else {
            if (!queryAttack(os, is, choice)) return false;
            else return performShoot(os, field, u, choice);
        }
...
bool checkWinning(ostream& os, Field& field, bool side)
{
    ...
    Vector<bool> sides_for_units = {};
    bool sample;
    for (int i = 0; i < h; ++i){
    for (int j = 0; j < w; ++j){
        Unit* temp = field.getUnit(i, j);
        if (temp != NULL) 
            sides_for_units.add(temp->getSide());
    }
    }
    if (sides_for_units.isEmpty()) sample = side;
    else {
        sample = sides_for_units[0];
        for (int i = 1; i < sides_for_units.size(); ++i)
            if (sides_for_units[i] != sample) return false; 
    }
    ...
    for (int j = 0; j < NU; ++j) {
        ...
        if (S == "A"){
            if (U == "FT"){
                ...
                assert (map->setUnit(row, col, temp)); 
            }
            else if (U == "KN"){
                ...
                Unit* temp = new Unit(ARCHER, true, row, col);
                assert (map->setUnit(row, col, temp));  
            }
        }
        else if (S == "B"){
            if (U == "FT"){
                ...
                Unit* temp = new Unit(ARCHER, false, row, col);
                assert (map->setUnit(row, col, temp));  
            }
        }
    }
